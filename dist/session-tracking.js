import { writeFileSync, existsSync, mkdirSync, readdirSync, statSync, openSync, readSync, closeSync, copyFileSync, readFileSync, } from "fs";
...
// Paths
// ---------------------------------------------------------------------------
export function getTrackingDir() {
 ...
 */
function resolveWindow(resetsAt, durationMs, nowMs) {
    if (resetsAt) {
 ...
 */
export async function performSessionTracking() {
    ensureDirs();
    if (!acquireLock())
    ...
    try {
        ...
        record.windows.seven_day = buildWindowRecord(usageData?.seven_day?.resets_at, SEVEN_DAY_MS, nowMs);
        // Per-model 7-day windows (only when the API surface includes them)
        if (usageData?.seven_day_opus) {
            record.windows.seven_day_opus = buildWindowRecord(usageData.seven_day_opus.resets_at, SEVEN_DAY_MS, nowMs);
        }
        if (usageData?.seven_day_sonnet) {
            record.windows.seven_day_sonnet = buildWindowRecord(usageData.seven_day_sonnet.resets_at, SEVEN_DAY_MS, nowMs);
        }
 ...
 */
export function triggerSessionTracking() {
    if (!isStale())
        return;
    const child = spawn(process.execPath, [join(__dirname, "index.js"), "--track-sessions"], {
        detached: true,
        stdio: "ignore",
