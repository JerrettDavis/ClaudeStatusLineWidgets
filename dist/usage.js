...
function readCredentials() {
    ...
    if (plat === "darwin") {
        ...
        try {
            const hex = execSync('security find-generic-password -a "$(whoami)" -w -s "Claude Code-credentials"', { timeout: 2000, stdio: ["pipe", "pipe", "pipe"] }).toString().trim();
            if (hex) {
...
export function triggerBackgroundFetch() {
    ...
    const child = spawn(process.execPath, [join(__dirname, "index.js"), "--fetch-usage"], {
        detached: true,
        stdio: "ignore",
 ...
 */
export async function fetchAndCacheUsage() {
