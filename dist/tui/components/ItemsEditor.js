...
export function ItemsEditor({ line, lineIndex, onChange, onAddWidget, onDeleteLine, onBack }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Text, { bold: true, children: ["Line ", lineIndex + 1, " Widgets"] }), _jsxs(Text, { dimColor: true, children: ["a = add | d = delete | m = ", moveMode ? "stop moving" : "move mode", " | x = delete line | esc = back"] }), _jsxs(Box, { flexDirection: "column", marginTop: 1, children: [line.length === 0 && _jsx(Text, { dimColor: true, children: "(empty \u2014 press a to add widgets)" }), line.map((item, i) => {
                        const widget = getWidget(item.type);
                        const name = widget?.getDisplayName() ?? item.type;
                        const selected = i === safeIdx;
                        return (_jsxs(Box, { children: [_jsxs(Text, { color: selected ? "cyan" : undefined, bold: selected, children: [selected ? (moveMode ? "↕ " : "▸ ") : "  ", name] }), item.color && _jsxs(Text, { dimColor: true, children: [" [", item.color, "]"] })] }, item.id));
                    })] })] }));
}
