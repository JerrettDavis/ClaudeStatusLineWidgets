...
export function isHeadroomActive() {
...
export function triggerHeadroomFetch() {
    if (!isCacheStale())
        return;
    const child = spawn(process.execPath, [join(__dirname, "index.js"), "--fetch-headroom"], { detached: true, stdio: "ignore" });
    child.unref();
}
export async function fetchAndCacheHeadroom() {
    try {
        ...
        const cache = { fetchedAt: Date.now(), data: stats };
